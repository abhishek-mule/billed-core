...
For this repository, you can also use the helper script instead of generating the manifest manually:

```bash
python scripts/build_custom_image.py --load
```

It reads `.env`, writes a generated manifest to `generated/apps.json`, includes ERPNext automatically, bundles India Compliance by default, and includes `electrical_trader_pack` only when `ELECTRICAL_TRADER_PACK_REPO` is set.
# Build the image
