...
| `SITES_VOLUME`            | Host path mounted into `sites`   | `./data/sites`                           | When you want host-level persistence            |
| `ASSETS_VOLUME`           | Host path mounted into `sites/assets` | `./data/assets`                     | Optional direct access to built assets          |
| `FRAPPE_DOCKER_NETWORK`   | Stable Docker network name       | `frappe_docker`                          | When you want a fixed network instead of Compose-generated names |
...
| `FRAPPE_BRANCH`  | Frappe framework branch used for custom builds | `version-16` | Used by `scripts/build_custom_image.py` |
| `FRAPPE_PATH`    | Frappe framework repository for custom builds | `https://github.com/frappe/frappe` | Used by `scripts/build_custom_image.py` |
| `INDIA_COMPLIANCE_REPO` | Repo URL for India Compliance app | `https://github.com/resilient-tech/india-compliance` | Optional custom image bundling |
| `INDIA_COMPLIANCE_BRANCH` | Branch for India Compliance app | `version-16` | Optional custom image bundling |
| `ELECTRICAL_TRADER_PACK_REPO` | Repo URL for electrical trader app | - | Set this to bundle the app into your image |
| `ELECTRICAL_TRADER_PACK_BRANCH` | Branch for electrical trader app | `main` | Optional custom image bundling |
