        ...
        <div className="fixed inset-0 z-50 flex items-end bg-background/80 backdrop-blur animate-fade-in lg:items-center lg:justify-center" onClick={closeSuccess}>
          <div className="w-full rounded-t-3xl border border-border bg-card p-6 shadow-elegant animate-slide-up lg:max-w-md lg:rounded-3xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-5 flex gap-2">
              <button onClick={() => window.open(`/print/${success.id}`)} className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-input py-3 text-sm font-medium transition-base hover:bg-secondary"><Printer className="h-4 w-4" /> Print</button>
              <button onClick={() => window.open(`/share/${success.id}`)} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-gradient-primary py-3 text-sm font-medium text-primary-foreground transition-base hover:opacity-90">Send via WhatsApp</button>
            </div>
