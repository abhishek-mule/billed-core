...
export function PriorityAlert({ item }: PriorityAlertProps) {
  ...
  return (
    <div className="relative bg-warning-soft border border-warning/30 rounded-xl p-4">
      <button
      ...
      <div className="flex items-start gap-3">
        <span className="flex items-center justify-center w-8 h-8 rounded-lg bg-warning/15 text-warning flex-shrink-0">
          <AlertTriangle size={16} />
        </span>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold text-warning-foreground">{item.customerName}</p>
          <p className="text-2xl font-bold text-warning-foreground tabular-nums mt-0.5">
            {formatINR(item.amount)}
          </p>
          <p className="text-xs text-warning-foreground/70 mt-1">{reasonText}</p>
        </div>
        <Link
