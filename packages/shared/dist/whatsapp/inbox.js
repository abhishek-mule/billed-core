...
exports.sanitizeRaw = sanitizeRaw;
exports.buildInboxRows = buildInboxRows;
/**
...
function providerEventIdForMessage(msg) {
    return msg.id ?? 'no-id';
}
function providerEventIdForStatus(st) {
 ...
 */
function buildInboxRows(events, provider, rawEnvelope) {
    const rows = [];
    ...
    for (const ev of events) {
        if (ev.eventType === 'status') {
            for (const st of ev.statuses ?? []) {
                rows.push({
                    ...
                    status: 'queued',
                    payload: sanitizeRaw({ ...ev, statuses: [st] }),
        ...
        for (const msg of ev.messages ?? []) {
            rows.push({
                ...
                phone_number_id: ev.phoneNumberId,
                status: 'queued',
                payload: sanitizeRaw({ ...ev, messages: [msg] }),
                payload_raw: rawPayload,
            });
        }
    }
