...
const buildAutomationPlan_1 = require("./buildAutomationPlan");
...
function planToNextAction(item) {
    return {
        when: (0, buildAutomationPlan_1.formatPlanTime)(item.nextAction.scheduledAt) || item.status.replace('_', ' '),
...
function computeEmptyState(cash, plan, nextActionItem) {
