...
import { textBody, tsToIso } from './normalize'
...
export function createWhatsAppDomain(client: SupabaseClient): WhatsAppDomain {
  /**
   * Resolve the recovery attempt (collection_actions.id) that produced a
   * provided message. Tries the billzo id first, then the raw provider receipt
   ...
   */
  async function resolveAttemptForMessageId(msgId: string): Promise<string | null> {
   ...
   */
  async function resolveReplyContext(contextId: string): Promise<ReplyContext | null> {
    ...
    const attemptId =
      anyRow?.recovery_attempt_id ?? (await resolveAttemptForMessageId(contextId))
    return attemptId
      ? {
  ...
  async function updateDeliveryStatus(tenantId: string, st: NormalizedStatus): Promise<void> {
    if (!st.id) return
    ...
    let message: ReplyContext | null = withAttempt ?? null
    if (!message) {
      const { data: anyRow } = await client
        .from('whatsapp_events')
        .select('recovery_attempt_id, invoice_id, customer_id')
